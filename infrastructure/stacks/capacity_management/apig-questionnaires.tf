...
resource "aws_api_gateway_integration" "questionnaires_GET_integration" {
  ...
  type                    = "AWS_PROXY"
  uri                     = "arn:aws:apigateway:${var.aws_region}:lambda:path/2015-03-31/functions/arn:aws:lambda:${var.aws_region}:${data.aws_caller_identity.current.account_id}:function:${var.questionnaires_function_name}-${terraform.workspace}/invocations"
  depends_on = [
    aws_api_gateway_resource.questionnaires_resource,
...
resource "aws_lambda_permission" "questionnaires_lambda_permission" {
  ...
  function_name = "${var.questionnaires_function_name}-${terraform.workspace}"
