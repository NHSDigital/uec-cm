...
resource "aws_api_gateway_integration" "organisation_affiliations_GET_integration" {
  ...
  type                    = "AWS_PROXY"
  uri                     = "arn:aws:apigateway:${var.aws_region}:lambda:path/2015-03-31/functions/arn:aws:lambda:${var.aws_region}:${data.aws_caller_identity.current.account_id}:function:${var.organisation_affiliations_function_name}${local.workspace_suffix}/invocations"
  depends_on = [
    aws_api_gateway_resource.organisation_affiliations_resource,
...
resource "aws_lambda_permission" "org_affiliations_lambda_permission" {
  ...
  function_name = "${var.organisation_affiliations_function_name}${local.workspace_suffix}"
