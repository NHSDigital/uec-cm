...
resource "aws_api_gateway_integration" "healthcare_services_GET_integration" {
  ...
  type                    = "AWS_PROXY"
  uri                     = "arn:aws:apigateway:${var.aws_region}:lambda:path/2015-03-31/functions/arn:aws:lambda:${var.aws_region}:${data.aws_caller_identity.current.account_id}:function:${var.healthcare_services_function_name}${local.workspace_suffix}/invocations"
  depends_on = [
    aws_api_gateway_resource.healthcare_services_resource,
...
resource "aws_lambda_permission" "healthcare_services_lambda_permission" {
  ...
  function_name = "${var.healthcare_services_function_name}${local.workspace_suffix}"
