...
resource "aws_serverlessapplicationrepository_cloudformation_stack" "dynamo-connector" {
  name             = local.athena_dynamo_connect_cf_stack_name
  ...
  parameters = {
    AthenaCatalogName = local.athena_data_catalogue_lambda_name
...
resource "aws_athena_data_catalog" "athena_data_catalogue" {
  name        = local.athena_data_catalogue_name
  description = "Athena CM data catalog"
  type        = "LAMBDA"

  parameters = {
    "function" = "arn:aws:lambda:${var.aws_region}:${local.account_id}:function:${local.athena_data_catalogue_lambda_name}"
  }
