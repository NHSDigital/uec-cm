...
def get_record_by_id(id: str):
    ...
    oa_table = dynamodb.Table(get_table_name())
...
def add_record(item):
    dynamodb = get_table_resource()
    oa_table = dynamodb.Table(get_table_name())
    response = oa_table.put_item(Item=item, TableName=get_table_name())
    return response
...
def delete_record(id):
    dynamodb = get_table_resource()
    oa_table = dynamodb.Table(get_table_name())
    response = oa_table.delete_item(Key={"id": id}, TableName=get_table_name())
    return response
